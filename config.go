...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(homeBtcdDir)
...
func loadConfig() (*config, []string, error) {
	...
	// Get config from command line, and set default directories if non-empty.
	commandCfg := config{}
	commandParser := flags.NewParser(&commandCfg, flags.Default)
	_, err := commandParser.Parse()
	if err != nil {
		if e, ok := err.(*flags.Error); !ok || e.Type != flags.ErrHelp {
			commandParser.WriteHelp(os.Stderr)
		}
		return nil, nil, err
	}
	...
	funcName := "loadConfig"
	err = os.MkdirAll(homeBtcdDir, 0700)
	if err != nil {
	...
	if cfg.SatsNet {
		numNets++
		activeNetParams = &mainSatsNetParams
	}
	if cfg.SatsTestNet {
		numNets++
		activeNetParams = &testSatsNetParams
	}
	...
	if cfg.DisableRPC {
		...
		btcdLog.Infof("cfg.RPCUser = %s", cfg.RPCUser)
		btcdLog.Infof("cfg.RPCPass = %s", cfg.RPCPass)
		btcdLog.Infof("cfg.RPCLimitUser = %s", cfg.RPCLimitUser)
		btcdLog.Infof("cfg.RPCLimitPass = %s", cfg.RPCLimitPass)
	}
	...
	// set.
	if cfg.Generate && len(cfg.MiningAddrs) != 1 {
		str := "%s: the generate flag is set, but there are %d mining " +
			"addresses specified "
		err := fmt.Errorf(str, funcName, len(cfg.MiningAddrs))
		fmt.Fprintln(os.Stderr, err)
		fmt.Fprintln(os.Stderr, usageMessage)
		return nil, nil, err
	}

	// Ensure the mining address is generated by mining pubkey.
	bValid, err := ValidatePubKeyAddress(cfg.MiningPubKey, cfg.MiningAddrs[0], cfg.SatsNet)
	if !bValid {
		fmt.Fprintln(os.Stderr, err)
		return nil, nil, err
	}
...
func ValidatePubKeyAddress(pubKeyStr string, address string, isMainNet bool) (bool, error) {
    // 检查输入参数
...
func validateP2PKHAddress(pubKey []byte, address string, params *chaincfg.Params) (bool, error) {
    ...
    uncompressedPubKeyHash := btcutil.Hash160(pk.SerializeUncompressed())

    p2pkhComp, _ := btcutil.NewAddressPubKeyHash(compressedPubKeyHash, params)
    p2pkhUncomp, _ := btcutil.NewAddressPubKeyHash(uncompressedPubKeyHash, params)

    return address == p2pkhComp.EncodeAddress() || 
           address == p2pkhUncomp.EncodeAddress(), nil
}

func validateP2WPKHAddress(pubKey []byte, address string, params *chaincfg.Params) (bool, error) {
    pk, err := btcec.ParsePubKey(pubKey)
    if err != nil {
        return false, fmt.Errorf("failed to parse public key: %v", err)
    }
    ...
    return address == witnessAddr.EncodeAddress(), nil
}

func validateP2SHAddress(pubKey []byte, address string, params *chaincfg.Params) (bool, error) {
    pk, err := btcec.ParsePubKey(pubKey)
    if err != nil {
        return false, fmt.Errorf("failed to parse public key: %v", err)
    }
    ...
    p2shAddr, err := btcutil.NewAddressScriptHashFromHash(p2shHash, params)
    if err != nil {
        return false, err
    }

    return address == p2shAddr.EncodeAddress(), nil
}

func validateP2TRAddress(pubKey []byte, address string, params *chaincfg.Params) (bool, error) {
    pk, err := btcec.ParsePubKey(pubKey)
    if err != nil {
        return false, fmt.Errorf("failed to parse public key: %v", err)
    }
    ...
    return address == taprootAddr.EncodeAddress(), nil
}
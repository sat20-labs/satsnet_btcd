	...
	if cfg.Generate {
		...
		if cfg.MiningPubKey != "" {
			if len(cfg.MiningAddrs) == 0 {
				...
				cfg.miningAddrs = append(cfg.miningAddrs, addr)
			} else if len(cfg.MiningAddrs) == 1 {
				// Ensure the mining address is generated by mining pubkey.
				bValid, err := ValidatePubKeyAddress(cfg.MiningPubKey, cfg.MiningAddrs[0], activeNetParams.Params)
				if !bValid {
					fmt.Fprintln(os.Stderr, err)
					return nil, nil, err
				}
			...
			// set pubkey and addr after miner initiated
			if len(cfg.MiningAddrs) != 0 {
				str := "%s: the generate flag is set but there are %d mining " +
					"addresses specified, but no mining pubkey"
				err := fmt.Errorf(str, funcName, len(cfg.MiningAddrs))
				fmt.Fprintln(os.Stderr, err)
				fmt.Fprintln(os.Stderr, usageMessage)
				return nil, nil, err
...
func ValidatePubKeyAddress(pubKeyStr string, address string, params *chaincfg.Params) (bool, error) {
	...
	return address == addr2.EncodeAddress(), nil
}

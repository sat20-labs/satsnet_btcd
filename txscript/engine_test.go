...
func TestBadPC(t *testing.T) {
	...
	for _, test := range tests {
		vm, err := NewEngine(pkScript, tx, 0, 0, nil, nil, -1, wire.TxAssets{}, nil)
		if err != nil {
...
func TestCheckErrorCondition(t *testing.T) {
	...
	vm, err := NewEngine(pkScript, tx, 0, 0, nil, nil, 0, wire.TxAssets{}, nil)
	if err != nil {
		t.Errorf("failed to create script: %v", err)
	}
...
func TestInvalidFlagCombinations(t *testing.T) {
	...
	for i, test := range tests {
		_, err := NewEngine(pkScript, tx, 0, test, nil, nil, -1, wire.TxAssets{}, nil)

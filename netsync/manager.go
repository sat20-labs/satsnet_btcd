...
func (sm *SyncManager) startSync() {
	...
	for peer, state := range sm.peerStates {
		...
		if peer.LastBlock() == best.Height {
			log.Debugf("Adding peer %s to equal peers.", peer.String())
...
func (sm *SyncManager) handleNewPeerMsg(peer *peerpkg.Peer) {
	...
	log.Infof(" %s isSyncCandidate %v ", peer, isSyncCandidate)
...
func (sm *SyncManager) handleDonePeerMsg(peer *peerpkg.Peer) {
	...
	log.Infof(" %s has removed to peerStates", peer)
...
func (sm *SyncManager) handleBlockMsg(bmsg *blockMsg) {
	...
	if err != nil {
		...
		if _, ok := err.(blockchain.RuleError); ok {
			log.Warnf("Rejected block %v from %s: %v", blockHash,
				peer, err)
		} else {
...
func (sm *SyncManager) handleInvMsg(imsg *invMsg) {
	...
	for i, iv := range invVects {
		log.Debugf("Receive iv message %s %v", iv.Type.String(), iv.Hash)

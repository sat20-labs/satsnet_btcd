...
var (
	...
	// anchorTxInfoBucketName is the name of the db bucket used to house the
	// anchor tx info set.
	anchorTxInfoBucketName = []byte("anchortxinfo")
...
type SpentTxOut struct {
	...
	// Assets is the Tx Assets of the output.
...
func spentTxOutSerializeSize(stxo *SpentTxOut) int {
	...
	return size + compressedTxOutSize(uint64(stxo.Amount), stxo.Assets, stxo.PkScript)
}
...
func putSpentTxOut(target []byte, stxo *SpentTxOut) int {
	...
	return offset + putCompressedTxOut(target[offset:], uint64(stxo.Amount), stxo.Assets,
		stxo.PkScript)
}
...
func decodeSpentTxOut(serialized []byte, stxo *SpentTxOut) (int, error) {
	...
	amount, assets, pkScript, bytesRead, err := decodeCompressedTxOut(
		serialized[offset:])
	offset += bytesRead
	if err != nil {
...
func serializeUtxoEntry(entry *UtxoEntry) ([]byte, error) {
	...
	size := serializeSizeVLQ(headerCode) +
		compressedTxOutSize(uint64(entry.Amount()), entry.txAssets, entry.PkScript())

	// Serialize the header code followed by the compressed unspent
	// transaction output.
	serialized := make([]byte, size)
	offset := putVLQ(serialized, headerCode)
	offset += putCompressedTxOut(serialized[offset:], uint64(entry.Amount()), entry.txAssets,
		entry.PkScript())
...
func deserializeUtxoEntry(serialized []byte) (*UtxoEntry, error) {
	...
	amount, txAssets, pkScript, _, err := decodeCompressedTxOut(serialized[offset:])
	if err != nil {
...
// Anchor TX information

// -----------------------------------------------------------------------------
// Anchor TX information consists of the all anchor tx.
//
// The serialized format is:
//
...
func deserializeAnchorTxInfo(serializedData []byte) (*AnchorTxInfo, error) {
	...
	if len(serializedData) < serializedMinLen {
		return nil, database.Error{
			ErrorCode:   database.ErrCorruption,
			Description: "corrupt anchor tx info",
		}
	}
...
// dbPutBestState uses an existing database transaction to update the best chain
// state with the given parameters.
func dbPutAnchorTxInfo(dbTx database.Tx, anchorTxInfo *AnchorTxInfo) error {

	// Serialize the current best chain state.
	serializedData := serializeAnchorTxInfo(anchorTxInfo)

	// Store the current Anchor into the database.
...
// 	// Store the current best chain state into the database.
// 	return dbTx.Metadata().Put([]byte(anchorTxInfo.LockedTxid), serializedData)
// }

// dbFetchAnchorTxInfos uses an existing database transaction to retrieve
// all anchor tx infos from the database.
func dbFetchAnchorTxInfos(dbTx database.Tx) (*[]AnchorTxInfo, error) {
	// Fetch the serialized data from the database.
	// 获取指定的 Bucket
	anchorTxInfoBucket := dbTx.Metadata().Bucket(anchorTxInfoBucketName)
	if anchorTxInfoBucket == nil {
	...
	anchorTxInfos := make([]AnchorTxInfo, 0)
	// 遍历 Bucket 中的所有键值对
	anchorTxInfoBucket.ForEach(func(key, value []byte) error {
		anchorTxInfo, err := deserializeAnchorTxInfo(value)
		if err != nil {
			fmt.Printf("Error deserializing anchor tx info: %v", err)
			return err
		}

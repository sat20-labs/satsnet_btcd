...
// IsAnchorTx determines whether or not a transaction is a anchor tx.  A AnchorTx
// is a special transaction created by lnd that has no inputs in sats net, it can be
// verify with scrript in input in btc net.  This is
// represented in the block chain by a transaction with a single input that has
// a previous output transaction index set to 0xfffffffe along with a
// zero hash.
//
// This function only differs from IsAnchorTx in that it works with a raw wire
// transaction as opposed to a higher level util transaction.
func IsAnchorTx(msgTx *wire.MsgTx) bool {
	// A coin base must only have one transaction input.
	if len(msgTx.TxIn) != 1 {
		return false
	}
	...
	// a zero hash.
	prevOut := &msgTx.TxIn[0].PreviousOutPoint
	if prevOut.Index != wire.AnchorTxOutIndex || prevOut.Hash != zeroHash {
		return false
	}
...
func CheckTransactionInputs(tx *btcutil.Tx, txHeight, bestHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, wire.TxAssets, error) {
	...
	if IsAnchorTx(msgTx) {
		// Check anchor input
		err := anchortx.CheckAnchorTxValid(msgTx, bestHeight < txHeight)
		if err != nil {
...
func (b *BlockChain) checkConnectBlock(node *blockNode, block *btcutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
	...
	for _, tx := range transactions {
		txFee, feeAssets, err := CheckTransactionInputs(tx, node.height, b.BestSnapshot().Height, view,
			b.chainParams)
		if err != nil {
			return err
		}

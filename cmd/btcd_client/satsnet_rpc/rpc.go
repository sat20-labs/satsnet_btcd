...
func GetTx(txid string) (*btcutil.Tx, error) {
	...
	ret, err := client.GetRawTransaction(hash)
	if err != nil {
		return nil, err
	}
...
func GetTxVerbose(txid string) (*btcjson.TxRawResult, error) {
	...
	ret, err := client.GetRawTransactionVerbose(hash)
	if err != nil {
		return nil, err
	}
...
func isUtxoSpentInMempool(txid string, vout int) (bool, error) {
	...
	_, err = client.GetMempoolEntry(txid)
	if err == nil {
	...
	if err != nil {
		// 如果交易不存在，可能已经被花费
		// 使用 GetTxOut 进一步确认
		txOut, err := client.GetTxOut(hash, uint32(vout), true)
		if err != nil {
...
func EstimateSmartFee(confTarget int64, mode btcjson.EstimateSmartFeeMode) (*btcjson.EstimateSmartFeeResult, error) {
	return client.EstimateSmartFee(confTarget, &mode)
}
...
func SendRawTransaction(txHex string, allowHighFees bool) (*chainhash.Hash, error) {
	...
	return client.SendRawTransaction(msgTx, allowHighFees)
}
...
func GetRawTransaction(TxHash *chainhash.Hash) (*btcutil.Tx, error) {
...
func GetBlockStats(hashOrHeight interface{}, stats *[]string) (*btcjson.GetBlockStatsResult, error) {
	return client.GetBlockStats(hashOrHeight, stats)
}

func GetMempoolEntry(txid string) (*btcjson.GetMempoolEntryResult, error) {
	return client.GetMempoolEntry(txid)
}

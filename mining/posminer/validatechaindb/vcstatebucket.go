...
func (s *ValidateChainStore) GetState() ([]byte, error) {
	var statedata []byte
	err := s.db.View(func(tx *bolt.Tx) error {
		bkt := tx.Bucket(vcStateBucket)
		statedata = bkt.Get(stateKey)
		if statedata == nil {
			return errors.New("No state data")
		}
		return nil
	})
...
func (s *ValidateChainStore) SetState(stateData []byte) error {
	err := s.db.Update(func(tx *bolt.Tx) error {
		bkt := tx.Bucket(vcStateBucket)
		if err := bkt.Put(stateKey, stateData); err != nil {
